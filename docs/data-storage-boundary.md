...
- Framework global root: `~/Library/Application Support/Claw`
- Framework main database: `~/Library/Application Support/Claw/claw.sqlite`
- Framework files/blob root: `~/Library/Application Support/Claw/files`
- Workspace root: `.claw/`
- Host root: `~/Library/Application Support/<Host>`
...
`claw.sqlite` is the main framework database. It stores user-facing structured
...
records, prefer `claw.sqlite` with namespaced/prefixed tables.

## Secrets

Secrets are separate from the main database. Plaintext secrets never live in
`claw.sqlite`. The framework stores encrypted vault data in the secrets/vault
...
`.claw/` stores workspace-local framework files: manifests, compatibility
markers, intents, observed state, projections, design/style/template/reference
assets, and other source-like files that should travel with a workspace.
...
compatibility code, but new writes go to `.claw/`, `claw.sqlite`, or a canonical
sidecar under the framework global root.
