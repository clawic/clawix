...
/// Lucide `info` glyph, ported 1:1 from the lucide-icons/lucide
...
struct InfoIconShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        path.addPath(SVGPathBuilder.build("M12 16v-4"), transform: xform)
        path.addPath(SVGPathBuilder.build("M12 8h.01"), transform: xform)
        return path
    }
}
