...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="clawix-opencode-e2e-") as raw:
        ...
        try:
            ...
            ws.send_json({"schemaVersion": 1, "type": "requestRateLimits"})
            ws.recv_until(
            ...
            ws.send_json({"schemaVersion": 1, "type": "openSession", "sessionId": image_chat})
            ws.send_json({
                "schemaVersion": 1,
                "type": "sendMessage",
            ...
            ws.send_json({"schemaVersion": 1, "type": "openSession", "sessionId": slow_chat})
            ws.send_json({"schemaVersion": 1, "type": "sendMessage", "sessionId": slow_chat, "text": "slow turn"})
            time.sleep(0.1)
            ws.send_json({"schemaVersion": 1, "type": "interruptTurn", "sessionId": slow_chat})
            ws.recv_until(
            ...
            ws.send_json({"schemaVersion": 1, "type": "archiveSession", "sessionId": image_chat})
            ws.recv_until(lambda f: f["type"] == "sessionUpdated" and f["session"]["id"] == image_chat and f["session"]["isArchived"] is True)
            ws.send_json({"schemaVersion": 1, "type": "unarchiveSession", "sessionId": image_chat})
