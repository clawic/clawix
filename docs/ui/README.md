...
15. Keep geometry contracts current with `scripts/ui_geometry_contract_check.mjs`.
16. Keep UI implementation evidence output current with
   ...
   public checks.
17. Keep UI implementation phases current with
...
21. Keep rendered drift report routes current with
   `scripts/ui_rendered_drift_check.mjs`.
22. Keep gate wiring current with `scripts/ui_release_gate_check.mjs`; UI
   governance checks must stay in local test lanes and public CI.
23. Keep rendered geometry evidence contracts current with
   `scripts/ui_rendered_geometry_manifest_check.mjs`.
24. Keep copy contracts current with `scripts/ui_copy_governance_check.mjs`.
25. Keep performance budget contracts current with
   `scripts/ui_performance_budget_check.mjs`; budget flow references must match
   private baseline references.
26. Keep component extraction APIs current with
   `scripts/ui_component_extraction_check.mjs`.
27. Keep mechanical refactor evidence current with
   `scripts/ui_mechanical_equivalence_check.mjs`.
28. Keep visual authorization scopes current with
...
29. Keep visual change detectors current with `scripts/ui_visual_detector_check.mjs`.
30. Keep visual model authorization current with
   `scripts/ui_visual_model_allowlist_check.mjs`; the active model signal must
   identify an allowlisted visual model.
31. Keep visual guard failure diagnostics current with
   `scripts/ui_visual_guard_failure_check.mjs`.
32. Keep conceptual visual proposal records current with
   `scripts/ui_visual_proposal_check.mjs`.
33. Keep private artifacts out of the public repo with
   `scripts/ui_private_artifact_boundary_check.mjs`; public files may store
   aliases, manifests, hashes, and runner contracts only.
34. Keep visible source coverage current with `scripts/ui_surface_inventory_check.mjs`.
35. Keep private baseline coverage current with
   `scripts/ui_private_baseline_manifest_check.mjs`; the public repo stores only
   safe hashes, aliases, tolerances, and runner IDs.
36. Keep aggregate private visual validation current with
    `scripts/ui_private_visual_validation_manifest_check.mjs`.
37. When all private roots are available, verify visual evidence end to end with
