...
Mirrors `packages/ClawixCore/Sources/ClawixCore/BridgeModels.swift` and `BridgeProtocol.swift`. Protocol version is read from `bridgeSchemaVersion` (currently `1`). When the daemon advertises a higher version, the SPA shows an "Update Clawix" empty state.
