...
| UI performance budgets are defined by critical flow and derive from approved measured baselines. | [`docs/ui/performance-budgets.json`](ui/performance-budgets.json), [`macos/PERF.md`](../macos/PERF.md) | Baseline capture remains pending until approved by the user; no performance optimization is complete from static reading alone. |
