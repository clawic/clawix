...
`apps/macos/`. The repo-root `clawix/CLAUDE.md` owns project-wide
conventions; this one only adds what is specific to the macOS target.

## Scope

- The macOS target lives at `apps/macos/`. Sources at
  `apps/macos/Sources/Clawix/`. Build via `bash dev.sh` from the repo
...
- Cross-platform code shared with iOS lives at
  `apps/shared/`. Edit there only when the change applies to both
  platforms.
- Do NOT touch `apps/ios/` from a macOS task unless the change is
...
1. Read `apps/macos/PERF.md`. Find the symptom in the table.
...
`apps/macos/PERF.md`. New signposts go through
...
pin lives in `apps/macos/CLAWJS_VERSION` (one line, semver). The build
pipeline reads it via `apps/macos/scripts/_emit_version.sh`, exports
...
2. We update `apps/macos/CLAWJS_VERSION` in a single commit, with a
