...
extension ActiveSkillState {
    /// Build the wire payload (`ActiveSkill`) the daemon expects on
    /// `thread/start` / `turn/start`.
    func toWire() -> ActiveSkill {
        let wireParams: [String: JSONValue]?
        if let params = params {
            wireParams = params.mapValues { value in
                switch value {
                case .string(let s):    return .string(s)
                case .number(let n):    return .double(n)
                case .bool(let b):      return .bool(b)
                case .secretRef(let id): return .object(["$secret": .string(id)])
                }
            }
        } else {
            wireParams = nil
