...
/// Lucide `x` glyph, ported 1:1 from the lucide-icons/lucide
...
struct XIconShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        var path = Path()
        path.addPath(SVGPathBuilder.build("M18 6 6 18"), transform: xform)
        path.addPath(SVGPathBuilder.build("m6 6 12 12"), transform: xform)
        return path
    }
}
