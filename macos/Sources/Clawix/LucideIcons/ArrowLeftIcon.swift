...
/// Lucide `arrow-left` glyph, ported 1:1 from the lucide-icons/lucide
...
struct ArrowLeftIconShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        var path = Path()
        path.addPath(SVGPathBuilder.build("m12 19-7-7 7-7"), transform: xform)
        path.addPath(SVGPathBuilder.build("M19 12H5"), transform: xform)
        return path
    }
}
