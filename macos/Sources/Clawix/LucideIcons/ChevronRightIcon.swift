...
/// Lucide `chevron-right` glyph, ported 1:1 from the lucide-icons/lucide
...
struct ChevronRightIconShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        var path = Path()
        path.addPath(SVGPathBuilder.build("m9 18 6-6-6-6"), transform: xform)
        return path
    }
}
